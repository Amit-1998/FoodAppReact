...
function PlanDetail() {
    ...
    const { user } = useAuth();
    useEffect(async () => {
        const data = await axios.get(`https://myfoodie-backend.herokuapp.com/api/plans/${id}`)
        console.log(data.data.data);
        delete data.data.data["_id"]
        delete data.data.data["__v"]
        setplan(data.data.data)
        const reviews = await axios.get("https://myfoodie-backend.herokuapp.com/api/getReview/" + id);
    ...
    const handleClick = async () => {
        console.log(123645);
        const data = await axios.post("https://myfoodie-backend.herokuapp.com/api/reviews", {
            "review": review,
            "rating": rate,
            "user": user.user._id,
            "plan": id
