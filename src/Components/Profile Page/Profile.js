...
function Profile() {
    ...
    const nameEdit = async () => {
        await axios.patch('https://myfoodie-backend.herokuapp.com/api/users/login');
    }
    const handleClick = async () => {
        try {
            console.log(user.user._id);
            const data = await axios.patch("https://myfoodie-backend.herokuapp.com/api/users/" + user.user._id,
